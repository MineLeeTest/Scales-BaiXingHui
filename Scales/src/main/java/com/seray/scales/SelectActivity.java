...
public class SelectActivity extends BaseActivity {
     ...
     * 全部商品集合
     */
    private List<SjcProduct> mProList = new ArrayList<>();
    /**
     * 选中商品集合
     */
    private ArrayList<SjcProduct> selectedList = null;
        ...
        for (int i = 0; i < 5; i++) {
            ...
            for (int j = 0; j < 6; j++) {
                ...
                if (datePosition < mProList.size()) {
                    ...
                    SjcProduct bean = mProList.get(datePosition);

                    String content = bean.getGoodsName();
    ...
    protected void checkView() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            SjcProduct bean = (SjcProduct) btn.getTag(R.id.tag_second);
    ...
    private boolean isContained(SjcProduct bean) {
        for (int i = 0; i < selectedList.size(); i++) {
            SjcProduct product = selectedList.get(i);
            if (product.getGoodsName().equals(bean.getGoodsName())) {
                return true;
            }
        }
        return false;
    ...
    public void onClick(View v) {
        ...
        switch (tag) {
            ...
            case BTN_BACK_FLAG:
                ...
                break;
            default:
                SjcProduct bean = (SjcProduct) v.getTag(R.id.tag_second);
                boolean isSelected;
                if (bean != null) {
    ...
    protected void updateView(int curPage) {
        ...
        for (int i = 0; i < PLU_AMOUNT; i++) {
            ...
            if (index < mProList.size()) {
                ...
                SjcProduct bean = mProList.get(index);
     ...
     * 获取加载页数
     */
    protected int getPageNumber(List<SjcProduct> list) {
    ...
    private static class SelectHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (activity != null && msg.what == MSG_UPDATE_VIEW) {
