...
public class ImageVideoBanner extends FrameLayout implements ViewPager.OnPageChangeListener, ImageVideoFragment.OnVideoCompletionListener {
    ...
    public ImageVideoBanner(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ImageVideoBanner);
        period = typedArray.getInt(R.styleable.ImageVideoBanner_period, 5000);
        typedArray.recycle();
        initView(context);
    }

    private void initView(Context context) {
        View view = LayoutInflater.from(context).inflate(R.layout.banner_imge_video, null, false);
        mViewPager = view.findViewById(R.id.view_pager);
        FragmentManager supportFragmentManager = ((AppCompatActivity) context).getSupportFragmentManager();
        mAdapter = new ViewsPagerAdapter(supportFragmentManager);
